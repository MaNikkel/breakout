//bibliotecas Arduino e Serial
import processing.serial.*;
import cc.arduino.*;

//variáveis entrada
boolean esq, dir, restart;
//entrada do arduino
int eixoX = 0;
int valorX;

//declara objeto Arduino
Arduino arduino;
...
{
  //ENTRADA
  leituraArduino();//ler Arduino a cada frame
  if(esq)
     posX -= 10; 
  else if(dir)
    posX += 10;
  ...
  //se todos blocos foram destruidos, prepara o próximo nível
  if(verificaBlocos())
  {
     proximoNivel();
  }
  ...
  if(bPosX < 0 || bPosX > 790)//bateu do lado
    bVelX *= -1;
  if(bPosY < 0)//bateu em cima
    bVelY *= -1;
  if(bPosY > 590)//se tocou o chão
    gameOver = true;
...
//reinicialização de todas as variáveis em caso de game over
void restart()
{
  pontos = 0;
  posX = width/2 - 50;
